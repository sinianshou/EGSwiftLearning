        ...
        if n > 0 {
            ...
            for i in 0 ... m {
                ...
                for j in 1 ... n {
                    ...
                    //1.当 p[j - 1] 不为 *，且 s[i - 1] 与 p[j - 1] 匹配（包括字符相同和 p[j-1] 为 . ）时，P[i][j] = P[i - 1][j - 1]
                    //2.当 p[j - 1] 为 *，且 * 没有进行匹配时，P[i][j] = P[i][j - 2]
                    //3.当 p[j - 1] 为 *，且 * 进行至少一次匹配时，P[i][j] = P[i - 1][j] && (s[i - 1] == p[j - 2] || p[j - 2] == '.')
